...
class BertClassifierDecodeIntegrated(nn.Module):
    def __init__(self, output_layer_dim, max_length, device):
        super(BertClassifierDecodeIntegrated, self).__init__()
...
# rinna classifer が正しいかどうかのために作成したモデル．
class BertClassifierDecodeIntegrated2(nn.Module):# ここはpretraind_model的なのを継承sルウ．
        
    def __init__(self, output_layer_dim, max_length, device):
        super(BertClassifierDecodeIntegrated2, self).__init__()
    ...
    def _get_sent_token_vecs(self, vec, token_num):
        vecs = [vec[:,i+1,:].view(-1, 768) for i in range(token_num)]  # BOSは必要ないから +1. range(token_num) は 0~
    ...
    def forward(self, input_ids, pred_spans, token_num, span_available_indication_matrix, usage):
        ...
        # 隠れ層からそれぞれ トークンのベクトルを取得する
        vecs = self._get_sent_token_vecs(hidden_states[-1], token_num)  # vecs[MAX_LENGTH][batch][2816]
        #print(len(vecs))
        #print(vecs[0].shape)
        ...
        # vec_combination の組に対する操作: predicate -> [vec_combi,2], predicate_inside -> [vec_combi, 1], not_predicate -> [vec_combi, 0]
        zeros = torch.zeros((768*2+1)*len(input_ids)).reshape(len(input_ids), (768*2+1)).to(self.device)
