...
from utils.tools import filter_span
...
if __name__ == "__main__":
    print(sys.argv)
    DATA = DATA.sample(frac=0.002, random_state=0).reset_index(drop=True)
    ...
    if sys.argv[5] == 'valid':
        ...
        classifier.load_state_dict(torch.load(f'../../models/srl_base_{MAX_LENGTH}_enc{ENC_NUM}_eachEP.pth'))
    ...
    if sys.argv[5] == 'test':
        ...
        # モデルをロード
        ENC_NUM = int(sys.argv[3])
        classifier.load_state_dict(torch.load(f'../../models/srl_base_{MAX_LENGTH}_enc{ENC_NUM}_best.pth'))
    ...
    with torch.no_grad():
        for i, (batch_features, batch_preds, batch_token) in enumerate(dataset):
            ...
            for scores, pred_span, token_num in zip(out, batch_preds, batch_token):   # batchから取り出し， 各文章毎に行う．
                ...
                reshaped_scores=filter_span(reshaped_scores, pred_span, token_num, ID2LAB, LAB2ID, span_available_indication)
                span_possible_tuples = [(i, j) for i in range(token_num) for j in range(token_num)]
                #assert len(span_possible_tuples) == len(reshaped_scores.reshape(-1,len(ID2LAB)))
                #for span_idx, scores_of_a_span in zip(span_possible_tuples, reshaped_scores.reshape(-1,len(ID2LAB))):
                #    if span_available_indication[span_idx[0], span_idx[1]] != -1:
                #        for label, score in zip( arg_labels, scores_of_a_span ):    # argのラベルだけ取り出す
                #            if score != -10000:
                #                span_score_lists.append([span_idx[0], span_idx[1], label, score])
                for span_idx, scores_of_a_span in zip(span_possible_tuples, reshaped_scores.reshape(-1,len(ID2LAB))):
                    if not np.isnan(scores_of_a_span[span_idx[0], span_idx[1]]):
                        for label, score in zip( arg_labels, scores_of_a_span ):    # argのラベルだけ取り出す
                            span_score_lists.append([span_idx[0], span_idx[1], label, score])
                # filterにかけた後，貪欲法で値の大きいものから取っていく
                print(f'{i} : ', len(span_score_lists), f'token_num = {token_num}')
    ...
    if sys.argv[5] == 'valid':
        ...
        lf1.to_csv(f"../../report/Validation/base_{MAX_LENGTH}_enc{ENC_NUM}.csv", encoding='utf-8_sig')
    ...
    if sys.argv[5] == 'test':
        ...
        lf1 = cal_label_f1(predictions, span_answers, LAB2ID)
        lf1.to_csv(f"../../report/Test/base_{MAX_LENGTH}_enc{ENC_NUM}.csv", encoding='utf-8_sig')
