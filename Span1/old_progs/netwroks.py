...
class BertClassifier(nn.Module):
    ...
    def forward(self, input_ids, pred_spans, token_num, span_available_indication_matrix):
        ...
        for i, j in span_possible_tuples:   # span_possible_tuples[MAX_LENGTH][MAX_LENGTH]
            if span_available_indication_matrix[i,j] == 1:   # 使用する範囲かどうか
                ...
                span_vec_list.append(torch.zeros(768+2+1))
                continue
        ...
        results = [F.log_softmax(out, dim=1) if span_available_indication_matrix[i,j] == 1 else torch.ones(self.output_layer_dim)*-1
                   for out,(i,j) in zip(outs, span_possible_tuples) ]
        
        return results
    

class BertClassifierLessDim(nn.Module):
    def __init__(self, output_layer_dim, max_length, device):
        super(BertClassifierLessDim, self).__init__()
...
class BertClassifierDecode(nn.Module):
    ...
    def forward(self, input_ids, pred_spans, token_num, span_available_indication_matrix):
        ...
        # vec_combination の組に対する操作: predicate -> [vec_combi,2], predicate_inside -> [vec_combi, 1], not_predicate -> [vec_combi, 0]
        for i, j in span_possible_tuples:   # span_possible_tuples[token_num][token_num]
            if span_available_indication_matrix[i,j] == 1:   # 使用する範囲かどうか
