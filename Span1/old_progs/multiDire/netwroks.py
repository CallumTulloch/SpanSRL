...
class BertClassifier(nn.Module):
    ...
    def forward(self, input_ids, pred_spans, token_num, span_available_indication_matrix):
        ...
        vecs = self._get_final_vecs(hidden_states[-1], self.max_length)  # vecs[MAX_LENGTH][batch][768]
        ...
        #print()
        outs_vertical = [F.log_softmax(out, dim=1) for out,(i,j) in zip(outs, span_possible_tuples) if span_available_indication_matrix[i,j] == 1]
        outs_vertical = torch.cat([torch.unsqueeze(out,dim=0) for out in outs_vertical], dim=0).permute(1,0,2) # [batch][max][label]
        
        outs_horizon = torch.concat([out[:,:31].unsqueeze(1) for out in outs], dim=1)   # out.unsqueeze(1)=[batch][1][only_arg_label]
        outs_horizon = F.log_softmax(outs_horizon, dim=1)        # outs[b_idx, max, label]
        outs_horizon =  outs_horizon.permute(0,2,1)              # outs_horizon = [batch][label][max]
        return outs_vertical, outs_horizon
...
class BertClassifierDecode(nn.Module):
    ...
    def forward(self, input_ids, pred_spans, token_num, span_available_indication_matrix):
        ...
        # 隠れ層からそれぞれ トークンのベクトルを取得する
        vecs = self._get_final_vecs(hidden_states[-1], self.max_length)  # vecs[self.max_length][batch][768]
        ...
        outs_vertical = [F.log_softmax(out, dim=1) for out,(i,j) in zip(outs, span_possible_tuples) if span_available_indication_matrix[i,j] == 1]  #[max][batch][label]
        
        outs_vertical = torch.concat([out[:,:31].unsqueeze(1) for out in outs_vertical], dim=1)   # out.unsqueeze(1)=[batch][1][only_arg_label]
        outs_mix = F.log_softmax(outs_vertical, dim=1)        # outs[b_idx, max, label]
        #outs_mix = F.softmax(outs_vertical, dim=1)        # outs[b_idx, max, label]
        return outs_mix