...
def whether_to_stop(epoch):
    ...
    # 標準エラーを標準出力に統合して取得
    try:
        proc = subprocess.run(['python3', '../Decode/decode_base.py', f'{MAX_LENGTH}', f'{MAX_ARGUMENT_SEQUENCE_LENGTH}', f'{ENC_NUM}', f'{DATAPATH}', 'valid', f'{MODEL_NAME}'], stdout=subprocess.PIPE)
...
if __name__ == "__main__":
    ...
    for epoch in range(50):
        ...
        for i, (batch_features, batch_labels, batch_preds, batch_token) in enumerate(train_dataset):   # labelsはバッチ16こに対し128トークンのラベル
            ...
            outs = classifier(input_ids, pred_span, token_num, span_available_indication)# [batch][max][label]
    ...
    """
    try:
        proc = subprocess.run(['python3', '../Decode/decode_base.py', f'{MAX_LENGTH}', f'{MAX_ARGUMENT_SEQUENCE_LENGTH}', f'{ENC_NUM}', f'{DATAPATH}', 'test', f'{MODEL_NAME}'], stdout=subprocess.PIPE)
    except subprocess.CalledProcessError as e:
        # e.stderrでなくe.stdoutなのに注意．なお，e.outputでも可．
        print('ERROR:', e.stdout) # ERROR: b'cat: undefined.py: No such file or directory\n'
