...
def get_train_test(MAX_LENGTH, MAX_ARGUMENT_SEQUENCE_LENGTH, data, lab2id):
    ...
    # ohuchi_data
    labels_ids = [id for lab, id in lab2id.items() if lab not in ['F-A', 'F-P', 'V', 'O', 'N']]
    arg_info_list=[]
    for args, pred in zip(data['args'].tolist(), data['predicate'].tolist()):
        arg_info=[[role, pred['word_start'], pred['word_end'], find_combination_index(MAX_LENGTH, (int(pred['word_start']), int(pred['word_end'])))] for role in labels_ids]
        for arg in args:
    ...
    # form data 
    try :
        train_df = train_df.loc[:, ['sentence', 'predicate', 'label_id', 'directed_graph_matrix', 'span_answers', 'num_of_tokens', 'args']]
        test_df = test_df.loc[:, ['sentence', 'predicate', 'label_id', 'directed_graph_matrix', 'span_answers', 'num_of_tokens', 'args']]
        valid_df = valid_df.loc[:, ['sentence', 'predicate', 'label_id', 'directed_graph_matrix', 'span_answers', 'num_of_tokens', 'args']]
    except:
        train_df = train_df.loc[:, ['sentence', 'predicate', 'label_id', 'span_answers', 'num_of_tokens','args']]
        test_df = test_df.loc[:, ['sentence', 'predicate', 'label_id', 'span_answers', 'num_of_tokens', 'args']]
        valid_df = valid_df.loc[:, ['sentence', 'predicate', 'label_id', 'span_answers', 'num_of_tokens', 'args']]
...
def get_train_test_decode(MAX_LENGTH, MAX_ARGUMENT_SEQUENCE_LENGTH, data, lab2id):
    ...
    # form data 
    try :
        train_df = train_df.loc[:, ['sentence', 'predicate', 'directed_graph_matrix', 'span_answers', 'num_of_tokens', 'args']]
        test_df = test_df.loc[:, ['sentence', 'predicate', 'directed_graph_matrix', 'span_answers', 'num_of_tokens', 'args']]
        valid_df = valid_df.loc[:, ['sentence', 'predicate', 'directed_graph_matrix', 'span_answers', 'num_of_tokens', 'args']]
    except:
        train_df = train_df.loc[:, ['sentence', 'predicate', 'span_answers', 'num_of_tokens','args']]
        test_df = test_df.loc[:, ['sentence', 'predicate', 'span_answers', 'num_of_tokens', 'args']]
        valid_df = valid_df.loc[:, ['sentence', 'predicate', 'span_answers', 'num_of_tokens', 'args']]
