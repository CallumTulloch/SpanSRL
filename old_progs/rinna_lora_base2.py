...
from preprocess.base.netwroks import RinnaClassifierDecodeIntegrated2
...
BATCH_SIZE = 8
MODEL_NAME = 'rinna_lora_base_rinna2_difWay'
PRETRAINED_MODEL = "rinna/japanese-gpt-neox-3.6b"
...
def decode(dataset, data_df):
    ...
    with torch.no_grad():
        ...
        for i, (batch_features, batch_preds, batch_token) in enumerate(dataset):
            ...
            # get output
            out = classifier(input_ids, pred_span, token_num, span_available_indication, 'decode', device) # out[MAX*MAX][batch][labels]
...
if __name__ == "__main__":
    ...
    classifier = RinnaClassifierDecodeIntegrated2.from_pretrained(PRETRAINED_MODEL, OUTPUT_LAYER_DIM, MAX_LENGTH)
    ...
    for epoch in range(30):
        ...
        for i, (batch_features, batch_labels, batch_preds, batch_token) in enumerate(train_dataset):   # labelsはバッチ16こに対し128トークンのラベル
            ...
            outs = classifier(input_ids, pred_span, token_num, span_available_indication, 'train',device) # [batch][max][label]
