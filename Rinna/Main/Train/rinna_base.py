...
if __name__ == "__main__":
    ...
    classifier = RinnaClassifierDecodeIntegrated(OUTPUT_LAYER_DIM, MAX_LENGTH, device).to(device)
    # まずは全部OFF
    for param in classifier.parameters():
        param.requires_grad = False
    for param in classifier.rinna.layers[32].parameters():
        param.requires_grad = True
    for param in classifier.rinna.layers[33].parameters():
        param.requires_grad = True
    for param in classifier.rinna.layers[34].parameters():
        param.requires_grad = True
    for param in classifier.rinna.layers[35].parameters():
        param.requires_grad = True
    for param in classifier.my_linear.parameters():
        param.requires_grad = True
    for param in classifier.my_linear2.parameters():
        param.requires_grad = True

    #ここでは全部ちゃんと設定しなくてはならない．
    optimizer = optim.Adam([
        {'params': classifier.rinna.layers[32].parameters(), 'lr': 5e-4},
        {'params': classifier.rinna.layers[33].parameters(), 'lr': 5e-4},
        {'params': classifier.rinna.layers[34].parameters(), 'lr': 5e-4},
        {'params': classifier.rinna.layers[35].parameters(), 'lr': 5e-4},
    ...
    for epoch in range(25):
        ...
        if prev_f1 < f1:
            ...
            prev_f1 = f1
            print('Valid f1 = ',f1,'\n')
            torch.save(classifier.state_dict(), f"../../models/{MODEL_NAME}/rinna_base.pth")
            continue
    ...
    del classifier
    classifier = RinnaClassifierDecodeIntegrated(OUTPUT_LAYER_DIM, MAX_LENGTH, device).to(device)
    classifier.load_state_dict(f"../../models/{MODEL_NAME}/rinna_base.pth")
