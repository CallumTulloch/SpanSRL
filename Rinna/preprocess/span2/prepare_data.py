...
def get_train_test(MAX_LENGTH, data, lab2id):
    ...
    # 学習データ，テストデータ作成．
    train_df2 = pd.DataFrame(features, columns=['args', 'predicate', 'sentence', 'num_of_tokens'])
    ...
    features = []
    span_answers=[]
    for args, pred, sentence in zip(test_df['args'].tolist(), test_df['predicate'].tolist(), test_df['sentence'].tolist()):
        arg_info=[[role, pred['word_start'], pred['word_end'], find_combination_index(MAX_LENGTH, (int(pred['word_start']), int(pred['word_end'])))] for role in lab2id.values()]
        span_answer=[]
        for arg in args:
            arg_info[lab2id[arg['argrole']]] = [lab2id[arg['argrole']], int(arg['word_start']), int(arg['word_end']), find_combination_index(MAX_LENGTH, (int(arg['word_start']), int(arg['word_end']))) ]
        ...
        span_answers.append(span_answer)
    # 学習データ，テストデータ作成．
    test_df2 = pd.DataFrame(features, columns=['args', 'predicate', 'sentence', 'num_of_tokens'])
    ...
    features = []
    span_answers=[]
    for args, pred, sentence in zip(valid_df['args'].tolist(), valid_df['predicate'].tolist(), valid_df['sentence'].tolist()):
        arg_info=[[role, pred['word_start'], pred['word_end'], find_combination_index(MAX_LENGTH, (int(pred['word_start']), int(pred['word_end'])))] for role in lab2id.values()]
        span_answer=[]
        for arg in args:
            arg_info[lab2id[arg['argrole']]] = [lab2id[arg['argrole']], int(arg['word_start']), int(arg['word_end']), find_combination_index(MAX_LENGTH, (int(arg['word_start']), int(arg['word_end']))) ]
        ...
        span_answers.append(span_answer)
    # 学習データ，テストデータ作成．
    valid_df2 = pd.DataFrame(features, columns=['args', 'predicate', 'sentence', 'num_of_tokens'])
