...
#test_df = test_df.sample(frac=0.02, random_state=0).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    if sys.argv[6] == 'valid':
        print(len(valid_df))
        dataset = mk_dataset(valid_df, 1, FORM)
        # モデルをロード
    ...
    if sys.argv[6] == 'test':
        print(len(test_df))
        dataset = mk_dataset(test_df, 1, FORM)
        # モデルをロード
        ENC_NUM = int(sys.argv[3])
    ...
    if sys.argv[6] == 'valid':
        ...
        #fid_report = classification_report(span_answers, predictions)
    if sys.argv[6] == 'test':
