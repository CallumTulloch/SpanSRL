...
def get_train_test(MAX_LENGTH, data, lab2id):
    ...
    features = []
    span_answers=[]
    for args, pred, sentence in zip(data['args'], data['predicate'], data['sentence']):
        arg_info=[]
        span_answer=[]
        for arg in args:
            arg_info.append([lab2id[arg['argrole']], arg['word_start'], arg['word_end'], find_combination_index(MAX_LENGTH, (int(arg['word_start']), int(arg['word_end']))) ])
