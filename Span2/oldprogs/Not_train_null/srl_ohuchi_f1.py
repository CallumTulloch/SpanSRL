...
def whether_to_stop(epoch):
    ...
    try:
        proc = subprocess.run(['python3', '../Decode/decoder_ohuchi_faster.py', f'{MAX_LENGTH}', f'{MAX_ARGUMENT_SEQUENCE_LENGTH}', f'{ENC_NUM}', f'{DATAPATH}','bert', 'valid', f'{MODEL_NAME}'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output = proc.stdout
...
if __name__ == "__main__":
    ...
    """
    try:
        proc = subprocess.run(['python3', '../Decode/decoder_ohuchi_faster.py', f'{MAX_LENGTH}', f'{MAX_ARGUMENT_SEQUENCE_LENGTH}', f'{ENC_NUM}', f'{DATAPATH}', f'{FORM}','bert', 'test', f'{MODEL_NAME}'], stdout=subprocess.PIPE)
    except subprocess.CalledProcessError as e:
        # e.stderrでなくe.stdoutなのに注意．なお，e.outputでも可．
        print('ERROR:', e.stdout) # ERROR: b'cat: undefined.py: No such file or directory\n'
