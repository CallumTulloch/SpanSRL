...
from preprocess.mk_train_test import get_train_test
...
class BertClassifier(nn.Module):
    ...
    def forward(self, input_ids, args_dic, pred_spans, token_num, span_available_indication_matrix):
        ...
        zeros = torch.zeros(len(lab2id)*len(input_ids)).reshape(len(input_ids), len(lab2id)).to(device) # len(input_size) = batch_size
        ...
        #print(outs.shape)
        results=[]
        for b_idx, item in list(args_dic.items()):
            #print(item)
            for role, start, end, span_idx in item:
                results.append( F.log_softmax(outs[b_idx, :, role]) )  # outs[b_idx, :, role] = [Max]
        
        return torch.cat([result.unsqueeze(0) for result in results], dim=0) 
...
def whether_to_stop(epoch):
    ...
    torch.save(classifier.state_dict(), f"../../models/srl_ohuchi_eachEP.pth")
    # 標準エラーを標準出力に統合して取得
    all_loss=0
    with torch.no_grad():
        for i, (batch_features, batch_preds, batch_token, batch_args) in enumerate(valid_dataset):   # labelsはバッチ16こに対し128トークンのラベル
            ...
            target_ids = torch.cat(target_ids_lists,dim=0).to(device)   # target_ids[num_of_args] ... [14,55,67,33...] span_idx
            ...
            batch_loss = loss_function(outs, target_ids)  
            all_loss += batch_loss.item()
    global prev_loss
    global patience_counter

    if prev_loss > all_loss:
        patience_counter = 0
        prev_loss = all_loss
        print('Valid all_loss = ',all_loss,'\n')
        torch.save(classifier.state_dict(), f"../../models/srl_ohuchi_best.pth")
        return False

    elif (prev_loss <= all_loss) and (patience_counter < 4):   # 10回連続でall_lossが下がらなければ終了
        print('No change in valid all_loss\n') 
        patience_counter += 1     
        return False
    else: 
        print('Stop. No change in valid all_loss\n') 
        return True
...
if __name__ == "__main__":
    ...
    for epoch in range(50):
        ...
        for i, (batch_features, batch_preds, batch_token, batch_args) in enumerate(train_dataset):   # labelsはバッチ16こに対し128トークンのラベル
            ...
            span_available_indication = np.ones([MAX_LENGTH, MAX_LENGTH]) * -1
            for span in list(itertools.combinations_with_replacement(np.arange(token_num), 2)):
                if (span[1] - span[0]) <= MAX_ARGUMENT_SEQUENCE_LENGTH and span[1]>=span[0]:
                    span_available_indication[span[0]][span[1]] = 1
            #print(np.count_nonzero((span_available_indication >= 1)))
            input_ids = batch_features.to(device)         # input token ids
            pred_span = batch_preds.to(device)            # predicate span
    ...
    """
    try:
        proc = subprocess.run(['python3', '../Decode/decoder_ohuchi.py', f'{MAX_LENGTH}', f'{MAX_ARGUMENT_SEQUENCE_LENGTH}', f'{ENC_NUM}', f'{DATAPATH}', f'{FORM}', 'test'], stdout=subprocess.PIPE)
