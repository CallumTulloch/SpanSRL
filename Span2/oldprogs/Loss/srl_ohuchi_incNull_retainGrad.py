...
class BertClassifier(nn.Module):
    ...
    def forward(self, input_ids, args_dic, pred_spans, token_num, span_available_indication_matrix):
        ...
        results =  results.permute(2,0,1)     # for experiment
...
def whether_to_stop(epoch):
    ...
    with torch.no_grad():
        for i, (batch_features, batch_preds, batch_token, batch_args) in enumerate(valid_dataset):   # labelsはバッチ16こに対し128トークンのラベル
            ...
            for out, target in zip(outs, target_ids.permute(1,0)):
                # print(out.shape)
                # print(target.shape)
                batch_loss = loss_function(out, target) # outs:(label, batch, max) -> (batch,max)  target:(16) 16個の正解スパン（特定ラベルにおける）
    ...
    elif (prev_loss <= all_loss) and (patience_counter < 3):   # 10回連続でall_lossが下がらなければ終了
