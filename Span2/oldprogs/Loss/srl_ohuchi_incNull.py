...
from preprocess.mk_train_test_incNull import get_train_test
...
def whether_to_stop(epoch):
    ...
    torch.save(classifier.state_dict(), f"../../models/srl_ohuchi_loss2_eachEP.pth")
    # 標準エラーを標準出力に統合して取得
    all_loss=0
    with torch.no_grad():
        for i, (batch_features, batch_preds, batch_token, batch_args) in enumerate(valid_dataset):   # labelsはバッチ16こに対し128トークンのラベル
            ...
            target_ids = torch.cat([target_ids.unsqueeze(0) for target_ids in target_ids_lists], dim=0).to(device) 
            ...
            batch_loss = loss_function(outs.reshape(-1, 2809), target_ids.reshape(-1))  # outs:(batch, label, max) -> (batch*label, max), target:(batch*label)
    ...
    if prev_loss > all_loss:
        ...
        torch.save(classifier.state_dict(), f"../../models/srl_ohuchi_loss2_best.pth")
        return False
...
if __name__ == "__main__":
    ...
    for epoch in range(50):
        ...
        for i, (batch_features, batch_preds, batch_token, batch_args) in enumerate(train_dataset):   # labelsはバッチ16こに対し128トークンのラベル
            ...
            batch_loss = loss_function(outs.reshape(-1, 2809), target_ids.reshape(-1)) # outs:(batch, label, max) -> (batch*label,max)  target:(16, 31) -> (16*31)  one-hot: (16*31, max)
            batch_loss.backward() 
            all_loss += batch_loss.item()
            optimizer.step()
    ...
    """
    try:
        proc = subprocess.run(['python3', '../Decode/decoder_ohuchi_2.py', f'{MAX_LENGTH}', f'{MAX_ARGUMENT_SEQUENCE_LENGTH}', f'{ENC_NUM}', f'{DATAPATH}', f'{FORM}', 'test'], stdout=subprocess.PIPE)
